...
function getStreak(completions, habits) {
  ...
  for (let i = 0; i < 365; i++) {
    ...
    d.setDate(today.getDate() - i);
    const key = d.toISOString().split("T")[0];
    if ((completions[key] || []).length === habits.length) streak++;
    else break;
  }
  return streak;
}
...
function getBestStreak(completions, habits) {
  ...
  for (let i = 364; i >= 0; i--) {
    ...
    d.setDate(today.getDate() - i);
    const key = d.toISOString().split("T")[0];
    if ((completions[key] || []).length === habits.length) {
      current++;
...
export default function HabitTracker() {
  // TODO: fetch from GET /habits on mount
  ...
  // Shape: { "2025-03-28": ["habitId1", "habitId2"], ... }
  // TODO: fetch from GET /logs/range?start=...&end=... on mount
  ...
  const completePct = habits.length > 0
    ? Math.round((todayDone.length / habits.length) * 100) : 0;
  ...
  const toggleHabit = (id) => {
    // TODO: await fetch(`POST /logs/toggle`, { habit_id: id, log_date: todayKey })
    setCompletions((prev) => {
  ...
  const addHabit = () => {
    ...
    // TODO: const { id } = await fetch(`POST /habits`, { name: newName, icon: newEmoji })
    const id = Date.now().toString(); // replace with API-returned id
  ...
  const removeHabit = (id) => {
    // TODO: await fetch(`DELETE /habits/${id}`)
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", padding: "40px 20px 24px" }}>
        <h1 style={{ fontSize: 22, fontWeight: 600, margin: 0, color: "#fff" }}>Habits</h1>
      ...
      <div style={{ maxWidth: 420, margin: "0 auto", padding: "0 16px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {/* Streak Card */}
        <div style={{ backgroundColor: "#1c1c1e", borderRadius: 20, padding: "24px 20px 20px" }}>
          <div style={{ textAlign: "center", marginBottom: 20 }}>
            <div style={{ fontSize: 48, lineHeight: 1 }}>🔥</div>
            <div style={{ fontSize: 52, fontWeight: 700, color: "#fff", lineHeight: 1.1 }}>
              {getStreak(completions, habits)}
            </div>
            <div style={{ fontSize: 12, color: "#888", letterSpacing: 1.5, marginTop: 4 }}>DAY STREAK</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", borderTop: "1px solid #2a2a2a", paddingTop: 16 }}>
            {[
              { label: "TODAY",    value: `${todayDone.length}/${habits.length}` },
              { label: "COMPLETE", value: `${completePct}%` },
              { label: "BEST",     value: getBestStreak(completions, habits) },
            ].map((item, i) => (
              <div key={i} style={{ textAlign: "center", borderRight: i < 2 ? "1px solid #2a2a2a" : "none" }}>
          ...
          <div style={{ height: 6, backgroundColor: "#2a2a2a", borderRadius: 3, marginTop: 16, overflow: "hidden" }}>
            <div style={{
              height: "100%", width: `${completePct}%`,
              backgroundColor: "#f59132", borderRadius: 3, transition: "width 0.4s ease",
            }} />
          </div>
        </div>

        {/* Activity Grid */}
        <div style={{ backgroundColor: "#1c1c1e", borderRadius: 20, padding: "20px" }}>
        ...
        <div style={{ backgroundColor: "#1c1c1e", borderRadius: 20, padding: "20px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            ...
            <button onClick={() => setShowAdd(true)} style={{
              backgroundColor: "#2a2a2e", color: "#fff", border: "none",
              ...
              return (
                <div key={habit.id}
                  style={{
                    ...
                    padding: "12px 4px", borderBottom: "1px solid #2a2a2a", cursor: "pointer",
                  }}
                ...
                >
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", color: "#444", fontSize: 18, cursor: "pointer", padding: "4px 6px" }}
                  >×</button>
                </div>
              );
            })}
      ...
      {showAdd && (
        <div style={{
          ...
          <div style={{
            backgroundColor: "#1c1c1e", borderRadius: "24px 24px 0 0",
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {EMOJI_OPTIONS.map((em) => (
                  <button key={em} onClick={() => setNewEmoji(em)} style={{
                    ...
                    backgroundColor: "#2a2a2e", fontSize: 20, cursor: "pointer",
                  }}>{em}</button>
                ))}
              </div>
            </div>
            <div style={{ marginBottom: 24 }}>
              ...
              <input autoFocus value={newName}
                ...
                style={{
                  width: "100%", backgroundColor: "#2a2a2e", border: "none",
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setShowAdd(false)} style={{
                flex: 1, padding: "14px", backgroundColor: "#2a2a2e",
                color: "#888", border: "none", borderRadius: 14, fontSize: 15, cursor: "pointer",
              }}>Cancel</button>
              <button onClick={addHabit} style={{
                flex: 1, padding: "14px", backgroundColor: "#f59132",
